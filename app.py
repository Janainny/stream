...
def load_data():
    ...
    df = df.drop(columns=['Data', 'Data_add', 'WindSpeed', 'WindSpeed10Min', 'RainRate', 'ETMonth', 'RainStorm', 'Station', ])
...
folium.Marker([-7.000126399270931, -42.10095318786533], popup='Estação Meteorológica UAPP - IFPI', tooltip='Estação Meteorológica UAPP - IFPI').add_to(m)

mapa = st_folium(m, width=700)

st.write('---')
st.header('Umidade')
st.line_chart(data=leituras, x='Datetime', y='HumOut', color='#4169E1')
st.write(f'Umidade mínima: {leituras.HumOut.min()}%')
st.write(f'Umidade máxima: {leituras.HumOut.max()}%')

st.write('---')
st.header('Radiação solar')
